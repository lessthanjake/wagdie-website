...
export const RadioGroup: React.FC<RadioGroupProps> = ({ name, options, value, onChange, label }) => {
  return (
    <div className="flex flex-col gap-3">
      {label && (
        <span className="text-xs uppercase tracking-widest text-neutral-500 font-display">
          {label}
        </span>
      )}
      <div className="flex flex-col gap-2">
        {options.map((option) => (
          <label key={option.value} className="flex items-center gap-3 cursor-pointer group w-fit">
            ...
            <span className={`text-sm font-serif transition-colors duration-300 ${value === option.value ? 'text-neutral-200' : 'text-neutral-500 group-hover:text-neutral-400'}`}>
              {option.label}
            </span>
          </label>
        ))}

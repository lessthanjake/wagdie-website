...
export const Blockquote: React.FC<{ children: React.ReactNode, cite?: string }> = ({ children, cite }) => {
  return (
    <figure className="my-6">
      <blockquote className="border-l-2 border-soul-accent/40 pl-6 italic text-neutral-400 text-lg font-serif leading-relaxed relative">
        <span className="absolute -top-4 -left-3 text-4xl text-soul-accent/20 font-display">"</span>
        {children}
      </blockquote>
      {cite && (
        <figcaption className="mt-3 pl-6 text-sm font-display tracking-widest text-neutral-600 uppercase">
          — {cite}
        </figcaption>
      )}
    </figure>
...
export const List: React.FC<{ children: React.ReactNode, type?: 'ul' | 'ol' }> = ({ children, type = 'ul' }) => {
  if (type === 'ol') {
    return (
      <ol className="list-decimal list-inside space-y-2 text-neutral-400 font-serif marker:text-soul-accent marker:font-display">
        {children}
      </ol>
    );
  }
  return (
    <ul className="space-y-2 text-neutral-400 font-serif">

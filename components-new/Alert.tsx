  ...
  return (
    <div className={`relative w-full rounded-sm border p-4 ${styles[variant]} flex gap-4 items-start ${className}`}>
      ...
      <div className="space-y-1">
        {title && <h5 className="font-display uppercase tracking-wider text-sm font-bold leading-none">{title}</h5>}
        <div className="text-sm font-serif opacity-90 leading-relaxed">
          {children}
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-soul-950 border border-red-900/30 shadow-[0_0_30px_rgba(0,0,0,0.8)] p-6 animate-fade-in">
        <div className="flex flex-col gap-2 text-center mb-6">
            ...
            <h2 className="text-xl font-display uppercase text-neutral-200 tracking-widest">{title}</h2>
            <p className="text-neutral-400 font-serif text-sm">{description}</p>
        </div>

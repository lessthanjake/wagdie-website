...
export function StatEditor({
  ...
  if (!isEditMode || !isOwner) {
    return (
      <div className={`bg-black/40 border border-neutral-800 p-3 text-center ${className}`}>
        <p className="text-[10px] font-display uppercase tracking-widest text-neutral-600 mb-1">
          {label}
        </p>
  ...
  return (
    <div className={`bg-black/40 border p-3 text-center ${error ? 'border-red-500' : 'border-neutral-700'} ${className}`}>
      <label className="text-[10px] font-display uppercase tracking-widest text-neutral-600 mb-1 block">
        {label}
      </label>
      <input
        type="number"

...
export const Heading: React.FC<HeadingProps> = ({
  ...
  const combinedClassName = `${resolvedSize} font-display text-neutral-200 ${className}`.trim()
...
function getTextColorClass(variant: TextVariant, muted: boolean): string {
  if (variant === 'caption' || variant === 'tiny') {
    return muted ? 'text-neutral-600' : 'text-neutral-500'
  }
  return muted ? 'text-neutral-500' : 'text-neutral-400'
}
...
export const UiLabel: React.FC<UiLabelProps> = ({
  ...
  const combinedClassName = `text-caption tracking-widest uppercase text-neutral-500 font-eskapade inline-flex items-center gap-1 ${className}`.trim()
